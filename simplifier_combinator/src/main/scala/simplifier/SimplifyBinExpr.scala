...
object SimplifyBinExpr {
  ...
  //          Simplifier(BinExpr("+",
  //            Simplifier(BinExpr("+",
  //              Simplifier(BinExpr("*", x1, y1)),
  //              Simplifier(BinExpr("*", x2, y2)))),
  //            Simplifier(BinExpr("+",
  //              Simplifier(BinExpr("*", x2, y3)),
  //              Simplifier(BinExpr("*", x4, y4)))))
  //          )
        // distributive properties of "*":
        case (BinExpr("*", l, r), expr) if expr == l => Simplifier(BinExpr("*", BinExpr("+", r, IntNum(1)), l))
        case (BinExpr("*", l, r), expr) if expr == r => Simplifier(BinExpr("*", BinExpr("+", l, IntNum(1)), r))
        ...
        // commutative properties:
        case (e@BinExpr("-", exprL, exprR), expr) =>
          if (exprR == expr) Simplifier(exprL)
          else BinExpr("+", Simplifier(e), Simplifier(expr))
        case (expr, e@BinExpr("-", exprL, exprR)) =>
          if (exprR == expr) Simplifier(exprL)
          else BinExpr("+", Simplifier(expr), Simplifier(e))
      ...
      case ("**", left, right)    => (Simplifier(left), Simplifier(right)) match {
        ...
        case (IntNum(n), expr)   => if (n == 1) Simplifier(expr) else if (n == 0) IntNum(1)   else {
          val s = Simplifier(expr)
          if (s != expr) Simplifier(BinExpr("**", s, IntNum(n))) else BinExpr("**", s, IntNum(n))
        }
        case (expr, FloatNum(n)) => if (n == 1) Simplifier(expr) else if (n == 0) FloatNum(1) else {
          val s = Simplifier(expr)
          if (s != expr) Simplifier(BinExpr("**", s, FloatNum(n))) else BinExpr("**", s, FloatNum(n))
        }
        case (FloatNum(n), expr) => if (n == 1) Simplifier(expr) else if (n == 0) FloatNum(1) else {
          val s = Simplifier(expr)
          if (s != expr) Simplifier(BinExpr("**", s, FloatNum(n))) else BinExpr("**", s, FloatNum(n))
        }

        // power laws:
        //case (BinExpr("**", IntNum(x), IntNum(y)), expr) => Simplifier(BinExpr("**", IntNum(x), BinExpr("**", IntNum(y), expr))) // dla testow tylko
        case (BinExpr("**", l, r), expr) => Simplifier(BinExpr("**", l, BinExpr("*", r, expr))) // to powinno byc, tego powyzej -- nie
