  ...
  def apply(binExpression: BinExpr): Node = {
    (binExpression.op, binExpression.left, binExpression.right) match {
      ...
      case ("-", left, right)    => (Simplifier(left), Simplifier(right)) match {
        ...
        case (binExpression, IntNum(n))
          if n == 0  => binExpression
        case (binExpression, FloatNum(n))
          if n == 0 => binExpression
        case (IntNum(n), binExpression)
          if n == 0  => Simplifier(Unary("-", binExpression))
        case (FloatNum(n), binExpression)
          if n == 0 => Simplifier(Unary("-", binExpression))

        // właściwości rozdzielcze ("*")
        //parseString("2*x-x") mustEqual parseString("x")
        case (BinExpr("*", l, r), binExpression)
          if binExpression == l => Simplifier(BinExpr("*", BinExpr("-", r, IntNum(1)), l))
        case (BinExpr("*", l, r), binExpression)
          if binExpression == r => Simplifier(BinExpr("*", BinExpr("-", l, IntNum(1)), r))
        ...
        case (binExpressionLeft, binExpressionRight) =>
          ...
          if (left1 != binExpressionLeft || right1 != binExpressionRight) Simplifier(BinExpr("-", left1, right1))
            else BinExpr("-", left1, right1)
      }
      ...
      case ("*", left, right)    => (left, right) match {
        case (binExpression, IntNum(n)) =>
          if (n == 1) Simplifier(binExpression)
          else if (n == 0) IntNum(0) else {
            val s = Simplifier(binExpression)
            if (s != binExpression) Simplifier(BinExpr("*", s, IntNum(n)))
            else BinExpr("*", s, IntNum(n))
          }
        case (IntNum(n), binExpression) =>
          if (n == 1) Simplifier(binExpression)
        ...
        case (binExpression, FloatNum(n)) =>
          if (n == 1) Simplifier(binExpression)
            ...
            val s = Simplifier(binExpression)
            if (s != binExpression) Simplifier(BinExpr("*", s, FloatNum(n)))
            else BinExpr("*", s, FloatNum(n))
        }
        case (FloatNum(n), binExpression) =>
          if (n == 1) Simplifier(binExpression)
          else if (n == 0) FloatNum(0)
      ...
      case ("or", left, right) =>
        ...
        (left1, right1) match {
          ...
          case (binExpressionLeft, binExpressionRight)
            if binExpressionLeft == binExpressionRight => binExpressionLeft
          case (binExpressionLeft, binExpressionRight) =>
